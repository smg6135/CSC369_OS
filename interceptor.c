...
{
	/*reterieve from pid from current and delete the pid and exit*/
	del_pid(current->pid);
...
asmlinkage long interceptor(struct pt_regs reg) {
	...
	if (table[syscall_moni].monitored == 2){
		log_message(current->pid, reg.ax, reg.bx, reg.cs, reg.dx, reg.si, reg.di, reg.bp);
	}else if (table[syscall_moni].monitored == 1){
		if(check_pid_monitored(syscall_moni, current->pid) == 1){
			log_message(current->pid, reg.ax, reg.bx, reg.cs, reg.dx, reg.si, reg.di, reg.bp);
		}
	}
	spin_unlock(&my_table_lock);

	return table[syscall_moni].f(reg);
}
...
*/
 ...
 /* - Check for correct context of commands (-EINVAL):
 *     a) Cannot de-intercept a system call that has not been intercepted yet. 1 intercepted 0 not intercepted
 *     b) Cannot stop monitoring for a pid that is not being monitored, or if the 
 *        system call has not been intercepted yet.
*/
	spin_lock(&my_table_lock);
 ...
 *   you might be holding, before you exit the function (including error cases!).  */
	if(cmd == REQUEST_SYSCALL_INTERCEPT){
		...
		spin_lock(&sys_call_table_lock);
		set_addr_rw((unsigned long) sys_call_table);
		sys_call_table[syscall] = table[syscall].f; //retrieve original
		set_addr_ro((unsigned long) sys_call_table);
		spin_unlock(&sys_call_table_lock);
